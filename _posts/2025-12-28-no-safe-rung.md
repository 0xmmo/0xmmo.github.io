...
I think this framing is wrong, and not because juniors aren't being replaced (they are). It's wrong because it implies a ceiling, some rung on the seniority ladder where humans become irreplaceable. There isn't one.

AI isn't eating the bottom of the engineering org chart. It's eating the entire thing, starting from the bottom because that's the path of least resistance.
...
Then they look at current AI tools, note that they struggle with some of these tasks, and conclude that senior engineers are safe. This is like looking at the first iPhone in 2007 and concluding that mobile phones will never replace cameras or GPS devices.

The mistake is treating the current snapshot of AI capability as a statement about its ceiling. But there's a deeper mistake underneath: a misunderstanding of what intelligence *is* and why it mattered in the first place.

## The knowledge thesis

Here is my claim: **the only thing that levels up an engineer is knowledge.**

You might object: what about aptitude? What about raw intelligence? I'd argue aptitude is simply the speed at which you acquire and synthesize knowledge. A "10x engineer" isn't ten times smarter in some abstract sense. They've accumulated more relevant knowledge about systems, failure modes, patterns, and trade-offs, and they can retrieve and apply it faster.

Every meaningful jump in my own career came from knowledge accumulation. You debug your first production outage and learn how distributed systems actually fail. You ship a feature that collapses under load and learn why you should have used a queue. You read a codebase written by someone better than you and absorb patterns you couldn't have invented. You sit in a hundred architecture reviews and develop an intuition for what will and won't scale.

Senior engineers aren't valuable because they're *smarter*. They're valuable because they've *seen more*. They carry a huge internal library of patterns, failure modes, and solutions built from years of lived experience.

This is why LLMs are so dangerous to the profession. Not because they're smarter than us, but because they *know more than us*, and the gap is not close.
...
An LLM has absorbed the experience of something like a million staff engineer lifetimes. It hasn't just read about how to design a distributed cache; it's read every implementation, every benchmark, every failure story, every competing approach. It hasn't seen one way to structure a React app. It's seen every way anyone has ever structured a React app and written about it publicly.
...
This is true today. It's also irrelevant to the trajectory.

The reason an LLM doesn't know your codebase isn't that it *can't*. It currently lacks the interfaces to acquire that knowledge efficiently. Your codebase isn't hidden behind some fundamental cognitive barrier. It's hidden behind authentication screens, Slack threads, Jira tickets, internal wikis, and tribal knowledge that lives in people's heads because nobody wrote it down.

These are interface problems, not intelligence problems. And interface problems get solved.

Look at what's already happened in the last two years. AI went from "can barely write a function" to "can autonomously resolve 80% of real GitHub issues." Context windows went from 4K tokens to over a million. AI agents gained the ability to execute shell commands, browse documentation, run tests, and iterate on failures. Each of these was an interface problem that got solved.
...
We've achieved roughly half of this list and are working our way through the rest.
...
This is true, and it matters for estimating *timelines*. But it doesn't change the *trajectory*. Every year, the harder benchmarks get solved too. SWE-Bench Verified went from single digits to 80% in two years. There's no reason to believe SWE-Bench Pro won't follow a similar curve.
...
This tells us that current tools are frustrating to use. It doesn't tell us that future tools will be. The first generation of any technology is frustrating. The question isn't whether today's tools are perfect; it's whether their problems are solvable. Code that's "almost right but not quite" is a solvable problem, and it gets better with every model generation.
...
Software is pure thought-stuff. The inputs are knowledge and reasoning. The outputs are text files. There is no waiting for cells to divide. There is no supply chain for bits. By Amodei's own framework, software engineering is among the *most* vulnerable domains to AI disruption, precisely because it has the fewest physical-world bottlenecks.

Amodei estimates that powerful AI "could come as early as 2026" and acknowledges that "in the long run AI will become so broadly effective and so cheap that [comparative advantage] will no longer apply." The CEO of the company building the tools that are replacing us is admitting, in carefully hedged language, that the economic logic protecting human engineers has an expiration date.
...
This is economically sound and practically misleading. Yes, in theory, as long as there's a single task where a human's opportunity cost is lower than spinning up an AI, the human remains employable. In practice, the number of such tasks shrinks with every model release. And the economic logic only works if the remaining human-advantaged tasks are valuable enough to sustain employment. Being the cheapest option for 3% of a job description doesn't pay a mortgage.

LessWrong contributor *A_donor* crystallized this in a post arguing that SWE automation would crash the crypto market: the path isn't a dramatic overnight jump from "AI can't code" to "AI replaces all coders." It's a gradual squeeze where AI handles an increasing share of tasks, teams shrink, hiring freezes spread, and the remaining engineers compete for fewer and fewer roles that still require a human in the loop. The jobs don't disappear in a dramatic moment. They erode.
...
But here's the dark irony: by the time the pipeline problem manifests, five to ten years from now, AI may have advanced to the point where senior engineers aren't needed either. The pipeline problem assumes a future where senior human engineers are still valuable. If that assumption is wrong, the pipeline problem is moot. You don't need to train human replacements for roles that no longer exist.

## What's left

To be clear about what I'm *not* saying: AI won't replace all software engineers next quarter, current tools aren't sufficient, and the transition won't be clean or painless.

What I am saying is that the structural barriers people point to — domain knowledge, codebase familiarity, architectural judgment, communication skills — are not permanent moats. They are interface problems and knowledge problems. And we are building systems whose entire purpose is to solve interface problems and knowledge problems.
...
I'm a senior engineer at multiple YC companies, and I'm telling you: there is no safe rung on this ladder.

The only honest position is to acknowledge that we are in a profession with a visible expiration date. Not because we're bad at our jobs, and not because juniors are replaceable and seniors aren't. It's because the thing that made us valuable — accumulated knowledge and the ability to apply it — is exactly what LLMs already do better than us, and they keep improving.
...
The question isn't whether AI will replace senior engineers. It's when. And everything I can see — the data, the benchmarks, the trajectories, the people building these systems — says sooner than we think.
